...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let p = (input.uv - center) * aspect;
    let radius = length(p);
    // atan2 has an undefined origin on some backends; preserve the center explicitly.
    var angle = 0.0;
    if radius > 0.000001 {
        angle = atan2(p.y, p.x);
    }
    let sectors = clamp(round(globals.parameters[0].x), 2.0, 24.0);
    let wedge = 6.283185307 / sectors;
    let phase = globals.parameters[0].y + globals.time_seconds * globals.parameters[1].z;
    let folded = abs(fract((angle + phase) / wedge + 0.5) - 0.5) * wedge;
    let sample_angle = folded + globals.parameters[0].w + radius * globals.parameters[1].w;
    let warped = vec2(cos(sample_angle), sin(sample_angle)) * radius / max(globals.parameters[0].z, 0.25);
    let uv = mirror_uv(center + warped / aspect);
