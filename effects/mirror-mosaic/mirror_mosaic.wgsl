...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grid = rotate((input.uv - 0.5) * aspect, globals.parameters[0].z) / aspect + 0.5;
    let repeats = clamp(round(globals.parameters[0].xy), vec2(1.0), vec2(12.0));
    let phase = globals.time_seconds * globals.parameters[1].zw;
    let tile = mirror_uv(grid * repeats + phase);
    let uv = mirror_uv(globals.parameters[1].xy + (tile - 0.5) / max(globals.parameters[0].w, 0.25));
