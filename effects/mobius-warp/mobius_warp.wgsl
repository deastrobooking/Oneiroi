...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let angle = globals.parameters[0].w + globals.time_seconds * globals.parameters[1].z;
    let z = rotate((input.uv - center) * aspect * 2.0, angle) / max(globals.parameters[0].z, 0.25);
    // f(z) = z / (c*z + 1); soften the pole before dividing, never after.
    let denominator = complex_multiply(globals.parameters[0].xy, z) + vec2(1.0, 0.0);
    let softness = max(globals.parameters[1].w, 0.01);
    let norm = max(dot(denominator, denominator), softness * softness);
    let warped = complex_multiply(z, vec2(denominator.x, -denominator.y)) / norm;
    let uv = mirror_uv(center + clamp(warped, vec2(-4096.0), vec2(4096.0)) / (2.0 * aspect));
