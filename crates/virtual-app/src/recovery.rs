...
use virtual_core::Quantization;
use virtual_graph::ParameterValue;
...
impl RecoveryEntry {
    ...
    pub(crate) fn markers(&self) -> &[virtual_session::TimelineMarker] {
...
impl State {
    pub(crate) fn refresh_session_recoveries(&mut self) {
        match discover_workspace_recoveries(
            &self.workspace,
    ...
    pub(crate) fn export_project_take(&mut self, index: usize, archive: bool) {
        ...
        let destination_root = if archive {
            self.workspace.join(".virtual/archive")
        } else {
        ...
        let source_directory = super::paths::journal_directories(&self.workspace)
            .into_iter()
            .find(|directory| directory.join(&take.journal_file).is_file())
            .unwrap_or_else(|| self.workspace.join(".virtual/session"));
        match copy_take_bundle(take, &source_directory, &destination_root) {
            Ok(destination) => {
                self.session_recovery_status = format!(
                    "{} copy created at {}",
...
fn copy_take_bundle(
    take: &virtual_io::TakeMetadataProject,
    ...
    let destination =
        destination_root.join(format!("{}-{}", take.take_id, virtual_io::new_project_id()));
...
fn rename_take_metadata(
    takes: &mut [virtual_io::TakeMetadataProject],
    index: usize,
    name: &str,
) -> Option<String> {
...
fn remove_take_metadata(
    takes: &mut Vec<virtual_io::TakeMetadataProject>,
    index: usize,
) -> Option<virtual_io::TakeMetadataProject> {
    (index < takes.len()).then(|| takes.remove(index))
}

fn discover_workspace_recoveries(
    workspace: &Path,
    ...
    let mut rejected = 0;
    let mut foreign = 0;
    for directory in super::paths::journal_directories(workspace) {
        let (found, invalid, other) = discover_recoveries(&directory, active_journal, project_id)?;
...
mod tests {
    ...
    use virtual_io::new_project_id;
    use virtual_session::{CommandOperation, CommandOrigin, JournalWriter, StateCheckpoint};
    ...
    fn catalog_excludes_active_journal_and_replays_valid_entries() {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let directory = std::env::temp_dir().join(format!("virtual-recovery-catalog-{unique}"));
        ...
        writer
            .try_append(virtual_session::ShowCommand {
                sequence: 0,
                command_id: 1,
    ...
    fn take_metadata_can_be_renamed_and_unlinked_without_deleting_files() {
        let mut takes = vec![virtual_io::TakeMetadataProject {
    ...
    fn take_exports_are_unique_copies_and_preserve_the_source_bundle() {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("virtual-take-export-{unique}"));
        ...
        writer.flush().unwrap();
        drop(writer);
        let take = virtual_io::TakeMetadataProject {
            take_id: new_project_id(),
