...
use virtual_media::{CrossfadeBus, DeckId, FourDeckMixer};
use virtual_render::{
...
use virtual_session::CommandOperation;
...
pub(crate) struct StructuralSnapshot {
    ...
    solo: [bool; 4],
    bypassed: [bool; 4],
    effect_slots: [[virtual_render::EffectSlot; 3]; 4],
    mirror: [bool; 4],
    lfos: [DeckLfos; 4],
    ...
    for slot in 0..ui.master_effects.slots.len() {
        ...
        if let Some(ids) = text_at(parameters, &format!("{root}.parameter_ids")) {
            effect.parameters = ids
                .split('\u{1f}')
                .filter(|id| !id.is_empty())
                .map(|id| virtual_render::EffectParameterValue {
                    id: id.to_owned(),
...
fn diff_deck_effect_structure(
    commands: &mut Vec<CommandOperation>,
    root: &str,
    old_slots: [virtual_render::EffectSlot; 3],
    new_slots: [virtual_render::EffectSlot; 3],
    old_lfos: DeckLfos,
    new_lfos: DeckLfos,
) {
    for slot in 0..3 {
...
mod tests {
    ...
    fn emitted_paths_restore_concrete_structural_state() {
        ...
        desired.deck_packages[3].modulation[0] = virtual_render::DeckPackageModulationRoute {
            enabled: true,
            source: 7,
            parameter_key: virtual_core::effect_parameter_key("fractal-volume", "density"),
            amount: -0.4,
        };
