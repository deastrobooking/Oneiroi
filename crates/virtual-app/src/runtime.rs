...
use virtual_io::{TakeMetadataProject, new_project_id};
use virtual_render::LoweredRenderPlan;
use virtual_session::{
...
impl PerformanceRuntime {
    ...
    pub(crate) fn active_graph(&self) -> &virtual_graph::ProjectGraph {
    ...
    pub(crate) fn add_timeline_marker(&mut self, at: ShowTime, label: String) -> Result<()> {
        ...
        journal
            .try_marker(virtual_session::TimelineMarker { at, label })
            .context("enqueue timeline marker")
    }

    pub(crate) fn set_project_graph(
        &mut self,
        graph: virtual_graph::ProjectGraph,
        extent: [u32; 2],
    ) -> Result<()> {
        let registry = builtin_registry();
    ...
    fn replace_baseline(
        ...
        if let Some(writer) = &replacement {
            writer
                .try_checkpoint(virtual_session::StateCheckpoint {
                    after_sequence: None,
                    at,
                    state: state.clone(),
                })
...
mod tests {
    ...
    use virtual_session::recover_journal;
    ...
    fn persistent_runtime_journals_commands_and_checkpoints_off_thread() {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let directory = std::env::temp_dir().join(format!("virtual-runtime-journal-{unique}"));
        let mut runtime = PerformanceRuntime::new([1920, 1080]).unwrap();
        runtime
