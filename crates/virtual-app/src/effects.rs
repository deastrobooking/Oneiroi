...
const EFFECT_PATH_ENV: &str = "VIRTUAL_EFFECT_PATH";
...
impl EffectRegistryWorker {
    pub(crate) fn spawn() -> Self {
        ...
        let thread = thread::Builder::new()
            .name("virtual-effect-registry".to_owned())
...
pub(crate) fn effect_resource_roots(workspace: &Path) -> Vec<PathBuf> {
    ...
    for name in [EFFECT_PATH_ENV, "ONEIROI_EFFECT_PATH"] {
        if let Some(paths) = std::env::var_os(name) {
            roots.extend(std::env::split_paths(&paths).filter(|path| !path.as_os_str().is_empty()));
        }
    }
    for legacy in [false, true] {
        if let Some(user_root) = user_effect_root(legacy)
            && user_root.is_dir()
        {
...
fn user_effect_root(legacy: bool) -> Option<PathBuf> {
    #[cfg(target_os = "macos")]
    {
        std::env::var_os("HOME").map(|home| {
            PathBuf::from(home).join(if legacy {
                "Library/Application Support/Oneiroi/effects"
            } else {
                "Library/Application Support/VIRTUAL/effects"
            })
        })
    }
...
mod tests {
    ...
    fn bundled_effects_do_not_depend_on_the_launch_directory() {
        let unrelated_workspace = std::env::temp_dir().join(format!(
            "virtual-unrelated-launch-directory-{}",
            std::process::id()
        ));
    ...
    fn invalid_refresh_keeps_the_previous_descriptor_until_the_package_is_removed() {
        let root = std::env::temp_dir().join(format!(
            "virtual-last-known-effect-root-{}",
            std::process::id()
        ));
    ...
    fn invalid_high_priority_package_keeps_precedence_over_a_duplicate() {
        ...
        let temp_root = std::env::temp_dir().join(format!(
            "virtual-duplicate-lkg-root-{}-{nonce}",
            std::process::id()
        ));
