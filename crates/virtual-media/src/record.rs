...
impl CameraRecorder {
    pub fn start(path: PathBuf, fps: u32) -> Result<Self, String> {
        ...
        let worker = thread::Builder::new()
            .name("virtual-camera-recorder".to_owned())
            .spawn(move || {
...
mod tests {
    ...
    fn records_rgba_frames_into_a_probeable_movie() {
        ...
        let path = std::env::temp_dir().join(format!(
            "virtual-camera-recording-{}-{stamp}.mov",
            std::process::id()
        ));
    ...
    fn capture_timestamps_preserve_dropped_frame_gaps_and_actual_duration() {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("virtual-record-timing-{stamp}.mov"));
    ...
    fn recording_directory_failure_is_reported_by_the_worker() {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let parent = std::env::temp_dir().join(format!("virtual-record-blocked-{stamp}"));
    ...
    fn dropped_tail_frames_extend_the_last_image_to_the_capture_endpoint() {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("virtual-record-tail-{stamp}.mov"));
