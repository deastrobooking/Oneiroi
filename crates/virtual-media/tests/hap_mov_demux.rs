...
use virtual_hap::{CompressedPlaneFormat, Decoder};
use virtual_media::{
...
impl Fixture {
    fn hap_mov() -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("virtual-hap-demux-{}-{id}.mov", std::process::id()));
        write_hap_mov(&path);
        Self { path }
    }
...
fn deck_worker_decodes_a_bounded_live_capture_source() {
    let config = CameraConfig {
        ...
        requested_fps: None,
        fps_denominator: 1,
        pixel_format: virtual_media::CapturePixelFormat::Auto,
    };
