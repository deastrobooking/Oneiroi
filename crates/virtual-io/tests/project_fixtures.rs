...
use virtual_graph::{CompileBudget, GraphCompiler, GraphRevision, builtin_registry};
use virtual_io::{
...
const V1_FIXTURE: &str = "tests/fixtures/project-v1.virtual";
const V2_FIXTURE: &str = "tests/fixtures/project-v2.virtual";
const V3_FIXTURE: &str = "tests/fixtures/project-v3.virtual";
const V4_FIXTURE: &str = "tests/fixtures/project-v4.virtual";
const V5_FIXTURE: &str = "tests/fixtures/project-v5.virtual";
...
fn v1_golden_project_migrates_to_current_and_round_trips() {
    let raw: serde_json::Value =
        serde_json::from_str(include_str!("fixtures/project-v1.virtual")).unwrap();
    ...
    let round_trip_path = std::env::temp_dir().join(format!(
        "virtual-project-v1-golden-{}.virtual",
        std::process::id()
    ));
...
fn v2_golden_project_migrates_to_current_and_round_trips() {
    let raw: serde_json::Value =
        serde_json::from_str(include_str!("fixtures/project-v2.virtual")).unwrap();
    ...
    let round_trip_path = std::env::temp_dir().join(format!(
        "virtual-project-v2-golden-{}.virtual",
        std::process::id()
    ));
    save_project_atomic(&round_trip_path, &project).unwrap();
    let reloaded: ProjectFile = load_project(&round_trip_path).unwrap();
...
fn v3_golden_project_migrates_to_current_and_round_trips() {
    let raw: serde_json::Value =
        serde_json::from_str(include_str!("fixtures/project-v3.virtual")).unwrap();
    ...
    let round_trip_path = std::env::temp_dir().join(format!(
        "virtual-project-v3-golden-{}.virtual",
        std::process::id()
    ));
    save_project_atomic(&round_trip_path, &project).unwrap();
    let reloaded: ProjectFile = load_project(&round_trip_path).unwrap();
...
fn v4_golden_project_migrates_to_current_and_round_trips() {
    let raw: serde_json::Value =
        serde_json::from_str(include_str!("fixtures/project-v4.virtual")).unwrap();
    ...
    let round_trip_path = std::env::temp_dir().join(format!(
        "virtual-project-v4-golden-{}.virtual",
        std::process::id()
    ));
    save_project_atomic(&round_trip_path, &project).unwrap();
    let reloaded: ProjectFile = load_project(&round_trip_path).unwrap();
...
fn v5_golden_project_migrates_to_v6_and_round_trips() {
    let mut source: ProjectFile =
        serde_json::from_str(include_str!("fixtures/project-v5.virtual")).unwrap();
    ...
    let round_trip_path = std::env::temp_dir().join(format!(
        "virtual-project-v5-golden-{}.virtual",
        std::process::id()
    ));
    save_project_atomic(&round_trip_path, &project).unwrap();
    let reloaded: ProjectFile = load_project(&round_trip_path).unwrap();
...
fn v6_golden_project_preserves_packages_modulation_and_clock_rig() {
    let project = load_project(fixture_path("tests/fixtures/project-v6.virtual")).unwrap();
    assert_eq!(project.version, PROJECT_VERSION);
    ...
    let key = virtual_core::effect_parameter_key("chromatic-split", "amount");
    ...
    assert_eq!(
        ...
        virtual_io::ClockSourceProject::MidiInput
    );
    ...
    let path = std::env::temp_dir().join(format!(
        "virtual-project-v6-golden-{}.virtual",
        std::process::id()
    ));
