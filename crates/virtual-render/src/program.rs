...
impl EffectReloadWorker {
    fn new(device: wgpu::Device, layout: wgpu::PipelineLayout) -> Self {
        ...
        let thread = thread::Builder::new()
            .name("virtual-effect-reload".to_owned())
...
impl MasterEffectProcessor {
    pub fn new(device: &wgpu::Device, program: &ProgramTarget) -> Self {
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("virtual-master-effects-pipeline-layout"),
        ...
        let passes = [
            make_pass(
                "virtual-master-slot-1-horizontal",
                &program.composition_view,
                &program.composition_view,
                &program.custom_history_views[0],
            ),
            make_pass(
                "virtual-master-slot-1-vertical",
                &program.composition_view,
                &program.scratch_a_view,
                &program.custom_history_views[0],
            ),
            make_pass(
                "virtual-master-slot-2-horizontal",
                &program.ping_a_view,
                &program.ping_a_view,
                &program.custom_history_views[1],
            ),
            make_pass(
                "virtual-master-slot-2-vertical",
                &program.ping_a_view,
                &program.scratch_a_view,
                &program.custom_history_views[1],
            ),
    ...
    fn draw_pass_with_pipeline(
        ...
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("virtual-master-effect-pass"),
...
impl ProgramPresenter {
    pub fn new(
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("virtual-program-presenter"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/present.wgsl").into()),
        });
        let layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("virtual-program-presenter-layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::FRAGMENT,
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("virtual-program-presenter-pipeline-layout"),
            bind_group_layouts: &[Some(&layout)],
            immediate_size: 0,
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("virtual-program-presenter-pipeline"),
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("virtual-program-presenter-sampler"),
            mag_filter: wgpu::FilterMode::Linear,
            min_filter: wgpu::FilterMode::Linear,
            ..Default::default()
        });
        let globals = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("virtual-program-presenter-globals"),
            size: size_of::<PresentGlobals>() as u64,
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("virtual-program-presenter-bind-group"),
            layout: &layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
    ...
    pub fn draw(
        ...
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("virtual-program-present-pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: target,
                depth_slice: None,
                resolve_target: None,
