...
fn require_kind(
    nodes: &BTreeMap<NodeId, &virtual_graph::CompiledNode>,
    node: NodeId,
    expected: &'static str,
...
mod tests {
    ...
    fn rejects_a_compiled_node_without_a_built_in_executor() {
        ...
        registry
            .register(NodeContract {
                ...
                ports: vec![virtual_graph::PortContract::output(
                    "video",
                    PortType::Texture2d,
                )],
                latency_frames: 0,
    ...
    fn rejects_a_shared_branch_that_cannot_represent_four_independent_decks() {
        ...
        graph.edges.extend([
            virtual_graph::Edge::new(2, "video", 20, "deck_b"),
            virtual_graph::Edge::new(2, "video", 20, "deck_c"),
            virtual_graph::Edge::new(2, "video", 20, "deck_d"),
        ]);
        let plan = GraphCompiler::new(&registry, CompileBudget::default())
            .compile(&graph)
            .unwrap();
