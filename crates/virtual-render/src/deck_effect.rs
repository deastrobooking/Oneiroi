...
impl DeckReloadWorker {
    fn new(
        ...
        let thread = thread::Builder::new()
            .name("virtual-deck-effect-reload".to_owned())
            .spawn(move || {
...
impl DeckEffectRuntime {
    pub(crate) fn new(device: &wgpu::Device, output_format: wgpu::TextureFormat) -> Self {
        let layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("virtual-deck-effect-layout"),
            entries: &[
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("virtual-deck-effect-pipeline-layout"),
            bind_group_layouts: &[Some(&layout)],
            immediate_size: 0,
        });
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("virtual-deck-effect-sampler"),
    ...
    pub(crate) fn create_passes(
        ...
        std::array::from_fn(|index| {
            let globals = device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("virtual-deck-effect-globals"),
                size: size_of::<DeckEffectGlobals>() as u64,
                usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
            });
            let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some(&format!("virtual-deck-effect-bind-group-{index}")),
                layout: &self.layout,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
    ...
    pub(crate) fn draw(
        ...
        let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("virtual-deck-effect-pass"),
